    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of NRC earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 NRC");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("NRC")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NRC, zNRC
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(19, 61, 85));
	set1->setColor(QColor(97, 213, 249));
