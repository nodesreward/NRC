...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("NodesReward client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("NodesReward"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("NodesReward"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
