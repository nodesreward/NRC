...
const char * const PIVX_CONF_FILENAME = "nrc.conf";
const char * const PIVX_PID_FILENAME = "nrc.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// NodesReward only features
// Masternode
...
// Mac: ~/Library/Application Support/nrc
// Unix: ~/.nrc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NodesReward";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\nrcParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\nrcParams
    // Mac: ~/Library/Application Support/nrcParams
    // Unix: ~/.nrc-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NodesRewardParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "NodesRewardParams";
#else
    // Unix
    return pathRet / ".nrc-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nrc.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# NodesReward config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nrc.conf
