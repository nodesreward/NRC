...
{
    // double check that there are no double spent zNRC spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zNRC serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zNRC spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zNRC check to not include duplicated serials in the same block.
