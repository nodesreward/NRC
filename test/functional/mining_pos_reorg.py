...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected NRC and zNRC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zNRCsupply']:
                assert_equal(node['zNRCsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check NRC and zNRC supply at the beginning
        # ------------------------------------------
        # zNRC supply: 2 coins for each denomination
        ...
        # NRC supply: block rewards minus burned fees for minting
        ...
        # Verify that NRC and zNRC supplies were properly updated after the spends and reorgs
        self.log.info("Check NRC and zNRC supply...")
